...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        {
            float tipTop = getUIValue(80.0f);
            float tipHeight = getUIValue(80.0f);
            _tipLabel = [self createLabel:self.view];
            _tipLabel.frame = CGRectMake(frameMove_X, tipTop, self.view.width - frameMove_X * 2, tipHeight);
            _tipLabel.textAlignment = NSTextAlignmentCenter;
            NSDictionary *infoDict = [NSBundle mainBundle].localizedInfoDictionary;
            if (!infoDict) infoDict = [NSBundle mainBundle].infoDictionary;
            ...
            if (iOS8Later)
            {
                ...
                float itemWidthSpacing = frameMove_X * 2;
                _settingBtn = [self createButton:self.view];
                [_settingBtn addTarget:self action:@selector(settingBtnClick) forControlEvents:UIControlEventTouchUpInside];
                _settingBtn.frame = CGRectMake(itemWidthSpacing, _tipLabel.bottom + frameMove_X, self.view.width - itemWidthSpacing * 2, itemHeight);
                [_settingBtn setTitleColor:WhiteColor forState:UIControlStateNormal];
                [_settingBtn setTitle:self.settingBtnTitleStr forState:UIControlStateNormal];
                _settingBtn.titleLabel.font = AppleFont_UltraLight(16);
                _settingBtn.backgroundColor = MainRoseRedColor;
                _settingBtn.layer.cornerRadius = itemHeight / 2;
            }
