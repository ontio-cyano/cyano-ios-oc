...
@interface CVEnterPasswordView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation CVEnterPasswordView
...
- (id)init
{
    ...
    {
        // Frame
        self.frame = CGRectMake(0.0f, MainAppDelegate.window.height, MainAppDelegate.window.width, MainAppDelegate.window.height);
        self.backgroundColor = MainViewsColor;
        
        // Pop View
        self.popView = [self createView:self];
        self.popView.frame = CGRectMake(0.0f, self.height, self.width, self.height);
...
{
    // 添加表
    self.customTableView = [self createTable:self frame:CGRectMake(0.0f, self.headerView.bottom, self.width, self.height - self.headerView.bottom) target:self];
...
- (void)layoutTableFooterView
{
    ...
    footerView.frame = CGRectMake(0.0f, 0.0f, self.width, footerHeight);
...
{
    self.customTableView.frame = CGRectMake(0.0f, self.headerView.bottom, self.width, self.height - self.headerView.bottom - height);
}

// (继承)
- (void)hideKeyboard
{
    self.customTableView.frame = CGRectMake(0.0f, self.headerView.bottom, self.width, self.height - self.headerView.bottom);
}
