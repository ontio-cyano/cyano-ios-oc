...
@interface CTNewAccountViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation CTNewAccountViewController
...
- (void)layoutHeaderView
{
    ...
    detailLabel.text = @"Enter your passphrase for account encryption.";
...
- (void)layoutTableView
{
    ...
    // Data
    self.customArray = @[@"Password", @"Password again"];
...
- (void)layoutTableFooterView
{
    ...
    // Items
    NSArray *itemsArr = @[@"SIGN UP", @"CANCEL"];
...
- (void)buttonItemPress:(UIButton *)button
{
    ...
    if (button.tag == 1)
    {
        ...
        NSString *pwdConfirm = cell_1.textField.text;
        ...
        ONTAccount *account = [[ONTAccount alloc] initWithName:@"" password:pwd];
        NSString *mnemonicText = [ONTMnemonicCode decryptMnemonicCode:account.encryptMnemonicText password:pwd address:account.address.address];
        ...
        if ([mnemonicText isEqualToString:account.mnemonicText])
        {
            ...
            // Login Success
            CTAccountSuccessViewController *controller = [[CTAccountSuccessViewController alloc] init];
