...
-(void)importOntId{
    if (self.pswField.text.length < 6||self.pswConfirmField.text.length < 6 ) {
        ...
        [CVShowLabelView showTitle:@"6 digits" detail:nil];
        return;
    }
    if (!_sureBtn.selected) {
        [CVShowLabelView showTitle:@"Please check box" detail:nil];
        return;
    }
    if (_textView.text.length !=52) {
        [CVShowLabelView showTitle:@"Failed to import ! Please input 52 characters ." detail:nil];
        return;
    }
    if ([self.pswConfirmField.text isEqualToString:self.pswField.text]) {
        [self toImportONTId];
    }else{
        [CVShowLabelView showTitle:@"The passwords do not match" detail:nil];
    }
}
-(void)toImportONTId{
