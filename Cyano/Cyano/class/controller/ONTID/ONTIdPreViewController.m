...
//
//    }else{
//        [CVShowLabelView showTitle:@"Wallet format error" detail:nil];
...
-(void)createONTId{
    ...
    [GCHApplication inputPassword:^{
        ...
        ONTIdentity *ONTId = [[ONTIdentity alloc]initWithName:@"" password:[GCHRAM instance].defaultAccount.password privateKeyHex:[GCHRAM instance].defaultAccount.privateKeyHex];
         [[CVRefreshVIew instance] stopRefresh];
        
        // 保存账号
        [GCHApplication saveDefaultONTId:ONTId password:[GCHRAM instance].defaultAccount.password];
